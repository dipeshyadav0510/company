...
const Navbar = () => {
  ...
  useEffect(() => {
    const handleStart = (url: string) => {
      // Only show loading bar if it's a new page, not a hash change
    ...
    document.addEventListener('navigationstart', () => handleStart(pathname))
    document.addEventListener('navigationend', handleComplete)
    document.addEventListener('navigationerror', handleComplete)

    return () => {
      document.removeEventListener('navigationstart', () => handleStart(pathname))
    ...
    <nav className="bg-gray-900 text-white relative">
      ...
      {isLoading && (
        <div className="absolute bottom-0 left-0 w-full h-1 bg-gray-700 overflow-hidden">
          <div 
            className="h-full bg-orange-500 transition-all duration-300 ease-in-out animate-loading-bar"
          />
        </div>
      )}
